...
class InnovaClimate : public climate::Climate, public Component {
 public:
   ...
   void update_climate_state() {
     ...
     this->dump_state_log_();

     changed |= this->update_mode_();
     changed |= this->update_action_();
     changed |= this->update_fan_mode_();
     ...
     if (call.get_mode().has_value()) {
       ...
       if (new_mode == climate::CLIMATE_MODE_OFF) {
         ...
         if (select_mode) {
           ...
           std::string opt = this->mode_to_string_(new_mode);
 ...
 protected:
   void dump_state_log_() {
     bool power_state = this->switch_power ? !this->switch_power->state : true;
     std::string mode_option =
         (this->select_mode && this->select_mode->has_state()) ? std::string(this->select_mode->current_option()) : "None";

     ESP_LOGD(TAG,
              "update_climate_state: switch_power_configured=%s, switch_power_state=%s (logical_power=%s), "
              "select_mode_has_state=%s, select_mode_option=%s",
              this->switch_power ? "YES" : "NO",
              this->switch_power ? (this->switch_power->state ? "ON (standby)" : "OFF (active)") : "N/A",
              power_state ? "ON" : "OFF", (this->select_mode && this->select_mode->has_state()) ? "YES" : "NO",
              mode_option.c_str());
   }
   ...
   bool update_fan_mode_() {
     if (this->select_fan && this->select_fan->has_state()) {
       climate::ClimateFanMode new_fan = this->fan_mode_from_string_(this->select_fan->current_option());
   ...
   climate::ClimateMode mode_from_string_(const std::string &m) {
     if (m == "Heating")
   ...
   climate::ClimateFanMode fan_mode_from_string_(const std::string &f) {
     if (f == "Night")
   ...
   std::string fan_mode_to_string_(climate::ClimateFanMode f) {
     if (f == climate::CLIMATE_FAN_LOW)
       return "Night";
     if (f == climate::CLIMATE_FAN_HIGH)
       return "Max";
     return "Auto";
   }
};
