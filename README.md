...
This repository contains an ESPHome configuration and a custom C++ component to integrate **Innova fancoils** (including **Innova**, **Airleaf**, **Filoterra**, and compatible units like **Olimpia Splendid Bi2**) using **PU / on-board control** or **M7** series boards with Home Assistant via Modbus RTU (RS-485) using an **M5Stack Atom Lite** and an **M5Stack RS485 Unit**.

The integration runs all climate control logic locally on the ESP32. It exposes a native Home Assistant `climate` entity, a local browser-based web panel, diagnostic sensors, and configuration entities.
...
> [!NOTE]  
> **Tested & Verified**: This integration has been physically tested and verified on the **Innova OSMO Series** fancoils. In theory, it is compatible with all other listed families and OEMs as they use the identical underlying M7/PU control boards and Modbus register maps.
> 
...
This integration is fully compatible with all fancoil models equipped with **Innova Electronics' M7 or PU control boards** (firmware default ID `1190` or compatible). These controllers are used across original Innova products as well as OEM models from other major manufacturers:

### 1. Supported Fancoil Families
* **Innova OSMO Series**: SL (Classic, 119 mm depth) & RS (Front radiant, 129 mm depth) (Sizes: 200, 400, 600, 800)
* **Innova FÄRNA Series**: Small (FARNASSC1II) & Big (FARNABSC1II) wall units
* **Innova Filomuro Series**: Slim Fit, Slim Fit XL, SWI (recessed flush-wall) (Sizes: 400, 600, 800, 800 XL)
* **Innova AirLeaf Series**: SL (exposed vertical/horizontal), SLI (recessed floor/wall), RSI (radiant recessed), SLSI (short height recessed)
* **Innova Filoterra Series**: Trench-recessed floor terminals
* **Innova Ducto / Ducto Multi**: Ducted horizontal/vertical units, Ducto Thin, Ducto Multi SLC+, Ducto Multi SLC+ Thin
* **Olimpia Splendid Bi2 Series**: Bi2 SLR (radiant panel), Bi2 Smart, Bi2 Wall, Bi2 Naked (SLN built-in), Bi2+, Bi2 Air
* **Vasco Series**: Vasco Viento, Vasco Niva FC
* **Other OEM Brands**: Buderus Logacool OEM units, Kermi hydronic fancoils, Termotech fancoils

### 2. Compatible Control PCB Boards
* **ESE845II / ESE846II** (Bluetooth modulating board)
* **ESE790** (Slim Fit specialized modulating board)
* **ESE745II / ESE746II** (M7 modulating receiver)
* **ESE645II** (Legacy modulating receiver)
* **INN-FR-B32** (Modbus retrofit board)

### 3. Compatible Touchpads & User Interfaces
...
2. **ESPHome Controller**: An ESP32 microcontroller (pre-configured for the **M5Stack Atom Lite**). In theory, any ESPHome-supported microcontroller board (ESP32/ESP8266) capable of Modbus RTU serial communication can be used.
...
* Connect the M5Stack RS485 Unit to the M5Stack Atom Lite's **Grove port** (Port A):
  * **TX Pin (GPIO 26)** $\rightarrow$ RS485 Unit TX
  * **RX Pin (GPIO 32)** $\rightarrow$ RS485 Unit RX
  * **5V Power** $\rightarrow$ RS485 Unit 5V
  * **GND** $\rightarrow$ RS485 Unit GND
...
> [!WARNING]  
> **Crossed Lines**: The RS485 transmission lines must be crossed (`A` to `B` and `B` to `A`) for communication to function.
...
1. Open your browser and navigate to `http://innova-<room_name>.local/` (e.g., `http://innova-bedroom.local/` if `room_name` is set to `bedroom`, or your device's IP address).
